...
### Frozen-byte publication sequence (v2.0.86 current candidate)
...
rebuilt or retagged. v2.0.86 is the required successor candidate and retains
...
```
   ...
   bare mirror must expose that exact object as `refs/tags/v2.0.86`, report type
...
The prerelease title must be exactly `Release v2.0.86`. Its body must be exactly
...
Artifact: Label_Match-v2.0.86.zip
...
2026-08-12 read-only preflight it is `false`; do not publish v2.0.86 until an
