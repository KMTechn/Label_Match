...
### Frozen-byte publication sequence (v2.0.85 current candidate)
...
partial bytes, logs, and evidence as immutable quarantined history. v2.0.85 is
the required successor candidate from that clean source and retains the
authoritative v2.0.83-aligned bounded diagnostic behavior introduced at commit
...
```
   ...
   `856b5e3b1ffeb341b54c825752cb8d88551d119dd498d33fb9589ec1c2121921`.
   ...
   bare mirror must expose that exact object as `refs/tags/v2.0.85`, report type
...
The prerelease title must be exactly `Release v2.0.85`. Its body must be exactly
...
Artifact: Label_Match-v2.0.85.zip
...
2026-08-12 read-only preflight it is `false`; do not publish v2.0.85 until an
