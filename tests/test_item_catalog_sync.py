...
PROFILE_CATALOG_ORIGIN = "https://server7.autoloop.test:18456"
...
def test_runtime_profile_origin_drives_default_catalog_route(monkeypatch, tmp_path):
    ...
    cache = tmp_path / "cache" / "Item.csv"
    bundle.write_bytes(CATALOG)
    profile_origin = "https://server5.autoloop.test:18456"
    calls = []

    monkeypatch.setattr(
        logistics_runtime_profile,
        "load_logistics_runtime_profile",
        lambda required=None: _profile(base_url=profile_origin),
    )
    ...
    assert calls == [
        (
            profile_origin + sync.CATALOG_PATH,
            2.0,
            False,
            {
                "Authorization": f"Bearer {SECRET_MARKER}",
...
def test_runtime_profile_rejects_foreign_url_without_transport(monkeypatch, tmp_path):
    bundle = tmp_path / "bundle.csv"
    bundle.write_bytes(CATALOG)
    calls = []
    ...
    monkeypatch.setattr(
        ...
        lambda required=None: _profile(
            base_url="https://server5.autoloop.test:18456"
        ),
    )
    ...
    with pytest.raises(
        ...
        refresh_item_catalog(
            ...
            url="https://foreign.example.invalid" + sync.CATALOG_PATH,
...
def test_production_profile_origin_still_allows_exact_catalog_route(
    monkeypatch, tmp_path
):
    bundle = tmp_path / "bundle.csv"
    cache = tmp_path / "cache" / "Item.csv"
...
def test_runtime_profile_rejects_near_match_url_without_transport(
    monkeypatch, tmp_path
):
    bundle = tmp_path / "bundle.csv"
    bundle.write_bytes(CATALOG)
    ...
    with pytest.raises(
        ...
        refresh_item_catalog(
            bundle,
            cache_path=tmp_path / "cache.csv",
            url="https://server5.autoloop.test:18457" + sync.CATALOG_PATH,
            get=lambda *args, **kwargs: calls.append((args, kwargs)),
        )
...
def test_catalog_failure_diagnostic_is_bounded_and_contains_no_secrets(
    ...
    class RejectedResponse:
        ...
        def raise_for_status(self):
            raise OSError(
                f"hidden {bearer_token} {source_host_id} {device_id}"
            )
    ...
    assert diagnostic["schema"] == sync.CATALOG_DIAGNOSTIC_SCHEMA
...
def test_authenticated_cache_survives_same_host_closed_port_outage(
    ...
    profile.base_url = "https://server7.autoloop.test:18458"
...
@pytest.mark.parametrize(
    "current_url",
    (
        "http://server7.autoloop.test:18458/inbound/api/item-catalog.csv",
        "https://different.autoloop.test:18458/inbound/api/item-catalog.csv",
        "https://operator@server7.autoloop.test:18458/inbound/api/item-catalog.csv",
        "https://operator:secret@server7.autoloop.test:18458/inbound/api/item-catalog.csv",
        "https://server7.autoloop.test:18458/catalog.csv",
        "https://server7.autoloop.test:18458/inbound/api/item-catalog.csv?download=1",
        "https://server7.autoloop.test:18458/inbound/api/item-catalog.csv#catalog",
        "https://server7.autoloop.test:/inbound/api/item-catalog.csv",
        "https://server7.autoloop.test:not-a-port/inbound/api/item-catalog.csv",
        "https://server7.autoloop.test:65536/inbound/api/item-catalog.csv",
    ),
)
def test_catalog_cache_authority_rejects_changes_beyond_valid_port(current_url):
    assert not sync._same_catalog_cache_authority(PROFILE_CATALOG_URL, current_url)
...
def test_verified_snapshot_parse_failure_reports_cause_and_exception(
    monkeypatch, tmp_path
):
    import Label_Match as app_module
    ...
    app = app_module.Label_Match.__new__(app_module.Label_Match)
    try:
        with pytest.raises(app_module.ItemCatalogSyncError) as raised:
            app._load_items_data()
    finally:
