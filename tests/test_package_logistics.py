...
def test_test1_prepare_ack_drop_is_one_shot_after_committed_exchange(
    monkeypatch,
):
    reconciliation_id = "PHSR-TEST1-ACK-LOSS"
    calls = []
    ...
    def transport(method, url, headers, body, timeout):
        ...
        return {
            ...
            "data": {
                "exchange": {
                    "exchange_id": "PHSX-TEST1-ACK-LOSS",
                }
            },
        }

    monkeypatch.setenv(
        package_module.TEST1_DROP_RECONCILIATION_PREPARE_ACK_ENV,
        reconciliation_id,
    )
    client = PackageLogisticsClient(
        PackageClientConfig(
            "https://logistics.test",
            "token",
            package_module.TEST1_GOAL_SCOPE_ID,
            "host",
            package_module.TEST1_COMMON_DEVICE_ID,
        ),
        transport=transport,
    )
    kwargs = {
        "authority_scope_id": package_module.TEST1_GOAL_SCOPE_ID,
        "action_ids": ["PHSA-1"],
        "expected_reconciliation_version": 7,
        "idempotency_key": "reconciliation-prepare-test1",
    }

    with pytest.raises(
        PackageTransportError,
        match="intentionally lost",
    ):
        client.prepare_phs_reconciliation_label_exchange(
            reconciliation_id,
            **kwargs,
    ...
    recovered = client.prepare_phs_reconciliation_label_exchange(
        reconciliation_id,
        **kwargs,
    )

    assert recovered["exchange"]["exchange_id"] == (
        "PHSX-TEST1-ACK-LOSS"
    )
    assert len(calls) == 2
...
@pytest.mark.parametrize(
    ("scope", "device_id", "env_value", "exchange_id"),
    [
        (
            "OTHER-SCOPE",
            package_module.TEST1_COMMON_DEVICE_ID,
...
def test_test1_prepare_ack_drop_requires_all_four_exact_gates(
    monkeypatch,
    scope,
    device_id,
    env_value,
