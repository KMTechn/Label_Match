...
import label_workbench_scan
from label_history import _label_match_summary_date
...
def _label_match_capture_current_set(current_set_info):
    current = current_set_info if isinstance(current_set_info, dict) else {}
    return {
        "full": copy.deepcopy(current),
        "id": str(current.get("id") or ""),
        "raw": tuple(current.get("raw") or ()),
        "parsed": tuple(current.get("parsed") or ()),
        "snapshot": copy.deepcopy(current.get("package_source_snapshot")),
    }


def _label_match_current_set_unchanged(current_set_info, captured):
    current = current_set_info if isinstance(current_set_info, dict) else {}
    expected = captured if isinstance(captured, dict) else {}
    return bool(
        current == expected.get("full")
        and str(current.get("id") or "") == expected.get("id", "")
        and tuple(current.get("raw") or ()) == expected.get("raw", ())
        and tuple(current.get("parsed") or ())
        == expected.get("parsed", ())
        and current.get("package_source_snapshot")
        == expected.get("snapshot")
    )


def _label_match_phs_replacement_notice_pair(resolution):
    scan = (
        resolution.get("scan")
        if isinstance(resolution, dict)
        and isinstance(resolution.get("scan"), dict)
        else {}
    )
...
class DataManager(_DataManager):
    """Keep the application constructor and its live dependency bindings."""

    _open_file = staticmethod(lambda *args, **kwargs: open(*args, **kwargs))
    _clock = staticmethod(lambda: datetime)
    _enrich_event = staticmethod(lambda *args: _enrich_label_match_event(*args))
    _datetime_encoder = staticmethod(lambda: DateTimeEncoder)
    _current_state_filename = staticmethod(lambda: Label_Match.FILES.CURRENT_STATE)
    _durable_event_types = staticmethod(lambda: LABEL_MATCH_DURABLE_EVENT_TYPES)
    _csv_safe_cell = staticmethod(lambda value: _csv_formula_safe_cell(value))
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            ...
            clock=self._clock,
            enrich_event=self._enrich_event,
            datetime_encoder=self._datetime_encoder,
            current_state_filename=self._current_state_filename,
            durable_event_types=self._durable_event_types,
            csv_safe_cell=self._csv_safe_cell,
        )
...
class Label_Match(tk.Tk):
    ...
    def __init__(self, run_tests=False):
        ...
        self._deferred_observability_status = None
        self._deferred_observability_review_baseline = None
        self._deferred_observability_auto_selected = False
        self._deferred_observability_read_in_progress = False
        self._deferred_observability_read_queue = queue.Queue(maxsize=1)
    ...
    def _async_load_history_task(self, result_queue, target_date=None, updates_active_state=None, load_generation=None):
        return label_history._async_load_history_task(
            self, result_queue, target_date, updates_active_state, load_generation,
            open_file=lambda *args, **kwargs: open(*args, **kwargs),
            clock=lambda: datetime,
            tray_complete_result=lambda details: _label_match_tray_complete_result(details),
            tray_complete_passed=lambda details: _label_match_tray_complete_passed(details),
            summary_date=lambda details: _label_match_summary_date(details),
            duplicate_index_barcodes=lambda details: _label_match_duplicate_index_barcodes(details),
        )
    ...
    def _format_deferred_age(seconds):
        ...
        days, remainder = divmod(value, 24 * 60 * 60)
    ...
    def _refresh_deferred_observability(self, *, validate_pending=False):
        ...
        if store is None:
            return None
        if self.__dict__.get("_deferred_observability_read_in_progress", False):
            return self.__dict__.get("_deferred_observability_status")
        review_baseline = self.__dict__.get(
            "_deferred_observability_review_baseline"
        )
        ...
        threading.Thread(
            target=worker,
            name="label-match-deferred-observability",
            daemon=True,
        ).start()
        if self.__dict__.get("_deferred_observability_poll_after_id") is None:
            self._deferred_observability_poll_after_id = self.after(
                25,
                self._poll_deferred_observability_readback,
            )
        return self.__dict__.get("_deferred_observability_status")

    def _poll_deferred_observability_readback(self):
        self._deferred_observability_poll_after_id = None
        if self.__dict__.get("_tk_shutdown_requested", False):
            self._deferred_observability_read_in_progress = False
        ...
        review_count = self._deferred_state_count(readback, "OPERATOR_REVIEW")
        if review_baseline is None or review_count < int(review_baseline):
    ...
    def _render_deferred_observability(self, readback):
        ...
        if tree is not None:
            try:
                for group in readback.operator_groups:
                    iid = f"deferred-{group.key}"
                    oldest_text = (
                        self._format_deferred_age(group.oldest_age_seconds)
                        if group.count
                        else "-"
                    )
                    if tree.exists(iid):
                ...
                selected = next(
                    ...
                    readback.operator_groups[0] if readback.operator_groups else None,
                )
                dependency_group = next(
                    (
                        group
                        for group in readback.operator_groups
                        if group.key == "dependency_wait" and group.count
                    ),
                    None,
                )
                selected = dependency_group or selected
                if selected is not None:
                    selected_iid = f"deferred-{selected.key}"
        ...
        oldest_text = (
            self._format_deferred_age(readback.oldest_age_seconds)
        ...
        if detail_widget is not None:
            delete = getattr(detail_widget, "delete", None)
            insert = getattr(detail_widget, "insert", None)
            if callable(delete) and callable(insert):
                try:
                    current_view = detail_widget.yview()
                except (TclError, AttributeError, TypeError):
                    current_view = ()
                try:
                    detail_widget.configure(state="normal")
                    delete("1.0", tk.END)
                    insert("1.0", detail_text)
                    detail_widget.configure(state="disabled")
                    if current_view:
                        detail_widget.yview_moveto(float(current_view[0]))
                except (TclError, AttributeError, TypeError, ValueError):
                    pass
            else:
                # Lightweight renderer fakes retain the previous Label-shaped
                # contract without weakening the bounded production widget.
                try:
                    detail_widget.configure(text=detail_text)
                except (TclError, AttributeError, TypeError):
                    pass
        ...
        alert_text = (
            "경보 "
            + " · ".join(
                alert_names.get(alert.code, alert.code) for alert in readback.alerts
            )
            if readback.alerts
            else "경보 없음"
        )
        alert_label = self.__dict__.get("deferred_observability_alert_label")
        if alert_label is not None:
            try:
                alert_label.configure(
                    ...
                    style=(
                        "Error.TLabel"
                        if any(alert.severity == "danger" for alert in readback.alerts)
                        else "Status.TLabel"
                    ),
                )
            except (TclError, AttributeError):
        ...
        if (
            readback.nonterminal_count
            and not self.__dict__.get("_deferred_observability_auto_selected", False)
        ):
            notebook = self.__dict__.get("operator_notebook")
            tab = self.__dict__.get("deferred_observability_tab")
            if notebook is not None and tab is not None:
                try:
                    notebook.select(tab)
                    self._deferred_observability_auto_selected = True
                except (TclError, AttributeError):
                    pass
        return readback
    ...
    def _show_deferred_validation_result(self, result):
        ...
        pending_count = int(getattr(result, "pending_count", 0) or 0)
        oldest_age = int(getattr(result, "oldest_age_seconds", 0) or 0)
        reason_code = operator_safe_reason_code(
        ...
        if state in {
            ...
            style = "Status.TLabel"
            tone = "primary"
        elif state in {"ACKED", "LOCAL_EFFECT_PENDING"}:
            headline = "서버확정-로컬반영대기"
            style = "Status.TLabel"
            tone = "primary"
            ...
            tone = "green"
        elif state in {"CANCELLED", "SUPERSEDED"}:
            headline = "종결-미완료"
            style = "Status.TLabel"
            tone = "primary"
        else:
        ...
        self._deferred_capture_ui = {
            ...
            "operator_complete_signal": state == "COMPLETED",
        }
    ...
    def _deferred_capture_pending_notice(self):
        ...
        if state.get("status") in {
            ...
            return WorkflowNotice(
                ...
                tone=(
                    "danger"
                    if state.get("status") == "관리자확인"
                    else "success"
                    if state.get("status") == "완료"
                    else "warning"
                ),
            )
    ...
    def _apply_resolved_central_phs2_seal(
        ...
        previous_active_label_id = str(
            current.get("active_label_id") or ""
        ).strip()
        if isinstance(sealed, dict):
            ...
            self._show_phs_replacement_required_notice_once(
                {
                    "scan": {
                        "replacement_required": True,
                        "scanned_label_id": previous_active_label_id,
                        "active_label_id": current.get("active_label_id"),
                    }
                }
            )
        ...
        *,
        ...
        def execute(event=None):
            ...
            self._start_phs_reconciliation_exchange(
                resolution,
                expected_current_set=captured_current_set,
            )
            return "break"
    ...
    def _start_phs_reconciliation_exchange(
        ...
        *,
        ...
        if (
            ...
            and not _label_match_current_set_unchanged(
                self.current_set_info,
                expected_current_set,
            )
        ):
            self._phs_label_guidance_notice = WorkflowNotice(
                title="현품표 교체 실행 취소",
                message=(
                    "조회 후 포장 상태가 바뀌어 교체 실행을 취소했습니다. "
                    "F5로 현품표를 다시 스캔하세요."
                ),
                kind="phs_label_exchange",
                tone="warning",
        ...
        def finish(payload):
            ...
            self._phs_label_guidance_notice = WorkflowNotice(
                ...
                message=(
                    "현품표 교체를 완료했습니다. 새 현품표별 작업을 "
                    "확인할 때까지 현재 포장 상태를 유지합니다."
                    if result.success
                    and working.get("phs_label_topology_refresh_required")
                    else (
                        "현품표 교체를 완료했습니다. 포장 수량과 진행 "
                        "상태는 유지됩니다."
                        if result.success
                        else (
                            "현품표 출력에 실패했습니다. 출력 상태를 "
                            "확인한 뒤 F5로 다시 시도하세요."
                        )
                    )
                ),
        ...
        *,
        ...
        def finish(payload):
            ...
            if kind != "result":
                ...
                messagebox.showerror(
                    "현품표 날짜 교환 실패",
                    "현품표 날짜 교환을 완료하지 못했습니다. 관리자에게 문의하세요.",
                    parent=self,
                )
                self._render_operator_workbench()
                self._focus_scan_entry_if_available()
            ...
            notice = WorkflowNotice(
                ...
                message=(
                    "현품표 날짜 교환을 완료했습니다. 포장 수량과 진행 상태는 유지됩니다."
                    if result.success
                    else "현품표 출력에 실패했습니다. 출력 상태를 확인한 뒤 F5로 다시 시도하세요."
                ),
                kind="phs_label_exchange",
                tone="success" if result.success else "danger",
            )
    ...
    def _handle_phs_label_exchange_shortcut(self, event=None):
        ...
        if coordinator.reconciliation_available():
            try:
                ...
                self._phs_label_guidance_notice = WorkflowNotice(
                    title="현품표 교체 복구 오류",
                    message="이전 현품표 교체 상태를 확인하지 못했습니다. 관리자에게 문의하세요.",
                    kind="phs_label_exchange",
                    tone="danger",
                )
                self._render_operator_workbench()
    ...
    def _queue_authoritative_package(
        ...
        *,
        ...
        return label_completion._queue_authoritative_package(
            self,
            item_code=item_code,
            is_manual_complete=is_manual_complete,
            current_set_info=current_set_info,
            persist_current_state=persist_current_state,
            logistics_runtime_required=lambda: logistics_runtime_required(),
            _central_scan_count=lambda: LABEL_MATCH_CENTRAL_INHERIT_ALL_SCAN_COUNT,
            _label_match_has_central_source_identity=lambda raw: _label_match_has_central_source_identity(raw),
            _label_match_parse_sealed_transfer_qr=lambda raw: _label_match_parse_sealed_transfer_qr(raw),
            _label_match_parse_new_format_fields=lambda raw: _label_match_parse_new_format_fields(raw),
            _label_match_existing_package_row_metadata=lambda *args: _label_match_existing_package_row_metadata(*args),
            _label_match_package_draft=lambda *args, **kwargs: _label_match_package_draft(*args, **kwargs),
            PackageLogisticsError=lambda *args: PackageLogisticsError(*args),
            OperationLeaseError=lambda *args: OperationLeaseError(*args),
    ...
    def _persist_ui_lane_current_set_snapshot(self, current):
        return label_completion._persist_ui_lane_current_set_snapshot(
            self,
            current,
            _clock=lambda: datetime,
        )
    ...
    def _commit_finalized_set_durable(
        ...
        *,
        ...
        return label_completion._commit_finalized_set_durable(
            self,
            details=details,
            item_code=item_code,
            is_manual_complete=is_manual_complete,
    ...
    def _apply_ui_lane_completion_snapshot(self, snapshot):
        return label_completion._apply_ui_lane_completion_snapshot(
            self,
            snapshot,
            deepcopy=lambda value: copy.deepcopy(value),
        )
    ...
    def _submit_finalized_set_on_lane(
        ...
        *,
        ...
        return label_completion._submit_finalized_set_on_lane(
            self,
            result=result,
    ...
    def _configure_base_styles(self):
        ...
        self.style.configure(
            "Deferred.Treeview.Heading",
            background=self.colors["heading_background"],
    ...
    def _apply_operator_responsive_layout(self, *, settle=False):
        ...
        try:
            ...
            deferred_detail_frame = self.__dict__.get(
            ...
            for name in (
                ...
                "deferred_observability_threshold_label",
            ):
                widget = self.__dict__.get(name)
                if widget is not None:
                    widget.configure(wraplength=deferred_detail_wrap)
    ...
    def _create_widgets(self):
        ...
        """
        profile, main_frame, panes = label_workbench_context.create_context_pane(
            self,
            tk=tk,
            ttk=ttk,
            TclError=TclError,
            build_operator_layout=build_operator_layout,
        ...
        label_workbench_scan.create_scan_status(
            ...
            tk=tk,
            ttk=ttk,
            LABEL_MATCH_CENTRAL_INHERIT_ALL_SCAN_COUNT=LABEL_MATCH_CENTRAL_INHERIT_ALL_SCAN_COUNT,
        )
        ...
        label_workbench_scan.create_scan_details(
            self,
            panes=panes,
            tk=tk,
            ttk=ttk,
        ...
        label_workbench_activity.create_activity_pane(
            self,
            profile=profile,
            panes=panes,
            tk=tk,
            ttk=ttk,
            DEFERRED_OPERATOR_STATUS_GROUPS=DEFERRED_OPERATOR_STATUS_GROUPS,
            DEFERRED_ALERT_THRESHOLDS=DEFERRED_ALERT_THRESHOLDS,
        )

        label_workbench_history.create_history_pane(
            self,
            tk=tk,
            ttk=ttk,
            create_completed_tray_cancel_button=lambda parent: ttk.Button(
                parent,
        ...
        label_workbench_context.create_status_footer(
            self,
    ...
    def _summary_items(scan_count):
        return label_history._summary_items(scan_count, clock=lambda: datetime)
