...
대상 이미지 폴더: `assets/label_match_user_manual_20260630`

작성 기준일: 2026-06-30

> **최신 기준(2026-06-30): 포장 1세트는 `현품표 1회 + 제품 4개 + 라벨지 1회 = 총 6스캔`입니다. `제품5`, `제품6`, `1/8~8/8` 기준은 최신 게시 승인 자료로 사용하지 않습니다.**
...
- 이미지 폴더: `Label_Match/docs/assets/label_match_user_manual_20260630`
...
1. `00_startup_idle.png`
2. `03_normal_scan_step_1.png`
3. `03_normal_scan_step_2.png`
4. `03_normal_scan_step_5.png`
5. `03_normal_scan_step_6.png`
6. `04_manual_complete_ready.png`
7. `13_mismatch_app_modal.png`
8. `07_current_set_cancel_before.png`
9. `11_completed_tray_cancel_confirm.png`
10. `17_restore_prompt.png`
11. `21_past_history_view.png`
...
| 이미지 업로드 방식 | 2026-06-27 게시 기록은 Chrome 편집기 PNG 붙여넣기 기준입니다. 2026-06-30 최신화분은 API 또는 OUTLINE 편집기에서 위 11개 이미지로 재업로드해야 합니다. |
| 본문 링크 치환 여부 | 기존 2026-06-27 게시: yes - 상대 이미지 링크 0개. 2026-06-30 로컬 최신화 문서: 로컬 상대 이미지 링크 11개, 누락 0개 |
| 이미지 렌더링 확인 | 기존 2026-06-27 게시: PASS - Markdown 이미지 참조 26개, 고유 attachment 24개. 2026-06-30 재게시 시 위 11개 첨부 기준으로 다시 확인 |
...
2026-06-30 로컬 최신화 결과:

- UI 캡처 evidence: `C:\company\program\_e2e_artifacts\label_match_display2_20260630_004846`
- 캡처 리포트: `label_match_operator_ui_walkthrough_report.json`
- 결과: `status=PASS`, `product_sample_count=4`, `total_scan_count=6`, 누락 효과음 없음
- 매뉴얼 자산 폴더: `Label_Match/docs/assets/label_match_user_manual_20260630`
- 시작, 설정, 현품표, 제품 1~4, 라벨지, 자동 완료, 수동 완료, 현재 세트 취소, 완료된 트레이 취소, 오류, 복구, 날짜 조회 화면을 보조 모니터 전체화면 기준으로 재촬영했습니다.
- 진행률은 마지막 라벨지 스캔 후 `6/6 통과 완료`까지 채워지는 기준입니다.

2026-06-29 로컬 UI 계약 보강:

- `완료된 트레이 취소 (F2)`는 일반 진행 버튼이 아니라 위험 작업 버튼 계약으로 분리했습니다.
- `Delete` 단축키는 왼쪽 기록 목록에 초점이 있을 때만 기록 삭제를 실행하도록 제한했습니다.
- 이 보강은 로컬 코드/테스트 계약 변경이며, 2026-06-30 로컬 UI 검증 증적을 production HTTPS, DB 반영, direct-sync 완료, 또는 새 Outline 재게시 증거로 확장하지 않습니다.
...
2. `python tools/publish_outline_user_manual.py --report-path docs/outline_user_manual_publish_result_20260630.json`을 실행합니다.
3. Chrome에서 문서를 강제 새로고침합니다.
4. Markdown 이미지 참조 11개, 고유 attachment 11개, ProseMirror 저장 본문 기준 `파일 업로드` 0개, 실제 `늘 버튼으로` 오탈자 0개를 확인합니다.
