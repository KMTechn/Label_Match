...
| OUTLINE document id | UI 생성 문서, API document id 미확인 |
| 게시 시각 | 2026-06-26 |
| 게시자 | Chrome 로그인 세션: 박관호 |
| 이미지 업로드 방식 | Chrome UI paste, 20 image blocks observed, full 21-image attachment verification pending |
| 본문 링크 치환 여부 | Markdown 본문 붙여넣기 완료, 상대 이미지 링크는 자동 렌더링되지 않음 |
| 이미지 렌더링 확인 | PARTIAL - Outline에서 다운로드 버튼 20개까지 확인, 21개 전체 순서 검증은 미완료 |
...
- Chrome 로그인 세션으로 직산 사업장 컬렉션에 `포장실 프로그램 사용 설명서` 문서를 생성하고 Markdown 본문을 붙여 넣었습니다.
- Chrome UI 이미지 붙여넣기는 동작했고, Outline 문서 안에서 이미지 다운로드 버튼 20개까지 확인했습니다.
- 이미지 21장 전체가 본문 순서대로 렌더링된다는 검증은 완료하지 못했습니다. 20개 이후 추가 paste가 카운트를 늘리지 않아 API attachment 업로드 또는 수동 확인이 필요합니다.
