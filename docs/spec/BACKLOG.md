...
- LM-C03 소스 교정: onboarding·GUI/guard·relay의 root resolver를 통일했다. fresh custom C는 ledger·registration `--sync-dir`도 C로 선택하고, 기존 ledger A가 있으면 A를 보존·진단한다. R1은 onboarding 상태를 ProgramData 과거 파일보다 먼저 감지하도록 교정하며, R2는 선택·분리 진단을 기본 WARNING 로그 채널에서 검증한다. 상태 없는 standalone의 ProgramData fallback과 새 PC의 최초 LOCALAPPDATA onboarding을 구별하며 settings template은 복사 전에 같은 값으로 해석한다. 검토의 일곱 원 fixture·기준 경로 대조, 두 process native callback 제외·해제와 split 설치의 원 데이터/identity/queue/spool 보존은 [Wave 3 수정 회귀](D:/KMTech/program-improvement-20260912/work/Label_Match/w3fix/RESULT.md)로 연결한다. guard 이전 ledger 초기화 순서, 실행 중 설정 변경/race, 모든 filesystem alias, 실제 GUI 복구·queue 재전송·enqueue/ACK는 **OPEN**이다. migration·프로필 재설계는 제공하지 않는다.
