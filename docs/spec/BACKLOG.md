...
- LM-C03 소스 교정: onboarding·GUI/guard·relay의 root resolver를 통일했다. fresh custom C는 ledger·registration `--sync-dir`도 C로 선택하고, 기존 ledger A가 있으면 A를 보존·진단한다. 기존 ProgramData standalone·LOCALAPPDATA onboarding과 첫 등록 전후 위치도 보존하며 settings template은 복사 전에 같은 값으로 해석한다. 두 process native callback 제외·해제와 split 설치의 원 데이터/identity/queue/spool 보존은 [Wave 3 host 회귀](D:/KMTech/program-improvement-20260912/work/Label_Match/w3/RESULT.md)로 연결한다. guard 이전 ledger 초기화 순서, 실행 중 설정 변경/race, 모든 filesystem alias, 실제 GUI 복구·queue 재전송·enqueue/ACK는 **OPEN**이다. migration·프로필 재설계는 제공하지 않는다.
