...
- 완료 존재 조회는 PC prefix별 재구축 가능한 SQLite 위치 색인을 사용한다. 실제 CSV 파일 집합·size·mtime·ctime이 모두 일치한 완전 coverage만 부재 판정에 쓰며, 색인 없음·손상·불일치는 전체 CSV 검색으로 재구축한다. 기존 완료의 flush→재대조→fsync는 유지한다.
...
- 주기 충돌 정리·조회와 workbench의 F4 상태 조회는 package worker에서 읽은 불변 snapshot을 Tk에서 적용한다. 현재 set·generation·업무 시작 epoch가 바뀐 결과는 무시하며 F3/F4 행동 직전의 정본 검사는 유지한다.
